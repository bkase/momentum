...
mod tests {
    ...
    fn test_update_entry() {
        ...
        // Add first entry
        index_manager.update_entry("active_session", &uuid1).unwrap();
        let index = index_manager.read_index().unwrap();
        ...
        index_manager.update_entry("active_session", &uuid3).unwrap();
        let index = index_manager.read_index().unwrap();
    ...
    fn test_migrate_from_vault_with_documents() {
        ...
        std::fs::write(docs_dir.join(format!("{}.md", reflection_uuid)), reflection_content).unwrap();
        ...
        std::fs::write(docs_dir.join(format!("{}.md", archived_uuid)), archived_content).unwrap();
