...
mod tests {
    ...
    async fn setup_large_vault() -> (TempDir, RealAethelStorage, Uuid) {
        ...
        std::fs::write(docs_dir.join(format!("{}.md", session_uuid)), session_content).unwrap();
        ...
        std::fs::write(docs_dir.join(format!("{}.md", checklist_uuid)), checklist_content).unwrap();
    ...
    async fn test_performance_improvement_find_active_session() {
        ...
        assert!(second_lookup_time < first_lookup_time * 3 / 4, 
            "Index lookup should be faster than linear search. First: {:?}, Second: {:?}",
            first_lookup_time, second_lookup_time);
    }
    ...
    async fn test_performance_improvement_get_checklist() {
        ...
        // The second lookup should be significantly faster
        println!("First checklist lookup (linear search): {:?}", first_lookup_time);
    ...
    async fn test_index_consistency_under_load() {
        ...
        for i in 0..10 {
            ...
            assert_eq!(session_result, Some(original_session_uuid), "Iteration {}", i);
            ...
            if i > 0 {
                ...
                assert_eq!(session_result, session_result2, "Session lookup inconsistent at iteration {}", i);
            }
        }
    }
    ...
    fn test_migration_performance_with_large_vault() {
        ...
        assert!(index.contains_key("active_session"), "Should find at least one session");
        assert!(index.contains_key("checklist"), "Should find at least one checklist");
        
        // Migration should complete reasonably quickly (under 1 second for 500 docs)
        assert!(migration_time.as_secs() < 2, "Migration should be fast, took {:?}", migration_time);
    }
}